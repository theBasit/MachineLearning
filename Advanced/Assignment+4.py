...
#     drop the None values from compliance columns
    ...
    clf_grid_search = GridSearchCV(clf, param_grid = grid_values, scoring = "roc_auc")
    clf_grid_search.fit(train,y_train)
    probs = clf_grid_search.predict_proba(test)[:,1]
...
get_ipython().run_cell_magic('time', '', 'blight_model()')
